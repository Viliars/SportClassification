...
class NextViT(nn.Module):
    def __init__(self, stem_chs, depths, path_dropout, attn_drop=0, drop=0, num_classes=1000,
        ...
        self.cache_output_channel = output_channel
    ...
    def replace_head(self, num_classes=30):
        self.proj_head = nn.Sequential(
            nn.Linear(self.cache_output_channel, num_classes),
        )
        trunc_normal_(self.proj_head[0].weight, std=.02)
        if hasattr(self.proj_head[0], 'bias') and self.proj_head[0].bias is not None:
            nn.init.constant_(self.proj_head[0].bias, 0)
